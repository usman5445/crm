...
export default function DataTableComponent(props) {
  ...
  useEffect(() => {
    ...
    usersData.error &&
      setSnackbarState({ open: true, message: usersData.error });
  }, [ticketData, usersData]);
  ...
  return (
    <Box sx={{ height: 500, width: "100%" }}>
      ...
      <DataGrid
        rows={
          (props.tableCase == "TicketTable" && ticketData.data) ||
          (props.tableCase == "UserTable" && usersData.data)
        }
        getRowId={(row) =>
        ...
        columns={cols}
        pageSize={pageSize}
        loading={ticketData.loading || usersData.loading}
